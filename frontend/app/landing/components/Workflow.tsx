...
const steps = [
  {
    ...
    panel: (
      <div className="bg-muted/20 backdrop-blur-sm p-4 rounded border border-border/50 text-[10px] font-mono text-muted-foreground">
  ...
  },
  {
    ...
    panel: (
      <div className="h-24 w-full rounded border border-border/50 bg-gradient-to-br from-orange-500/5 via-muted/20 to-muted/10 flex items-center justify-center">
    ...
    panel: (
      <div className="flex gap-2">
        {["Edge Computing", "Rust/WASM", "DX Strategy"].map((tag) => (
          <span key={tag} className="bg-muted/20 border border-border/50 px-3 py-1 rounded text-xs">{tag}</span>
        ))}
      </div>
    ),
  },
          ...
          <div className="space-y-24">
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className="absolute left-1/2 -translate-x-1/2 size-10 rounded-full bg-background border border-border/50 text-foreground flex items-center justify-center font-bold z-10 hidden md:flex">
                  {step.number}
                </div>
